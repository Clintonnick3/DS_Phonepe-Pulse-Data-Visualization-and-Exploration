...
st.sidebar.header(":violet[**Welcome to the Pulse dashboard**]")
...
if selected == "Home":
    ...
    with col1:
        ...
        st.markdown("### :violet[Mod used :] Github Cloning, Python, Pandas, MySQL, mysql-connector-python, Streamlit, and Plotly.")
        st.markdown("### :violet[Overview :] You can visualize :violet[phonePe pulse] data and learn a lot about transactions, user numbers, the top 10 states, districts, and pin codes, as well as which brand has the most users, by using this streamlit web application. To gain some insights, bar charts, pie charts, and geo map visualization are used.")
    with col2:
        st.image("pul.jpeg")
...
if selected == "Top Charts":
    ...
    """
    <style>
    /* Custom styles for the slider background */
    .st-b5 {
        background: #8A2BE2; /* Purple color */
        border-radius: 8px; /* Rounded corners */
    }
    /* Custom styles for the slider handle */
    .st-b5 .st-ce {
        background: #8A2BE2; /* Purple color */
    }
    /* Custom styles for the slider label text */
    .st-b5 .st-bj, .st-b5 .st-cc {
    ...
    with colum2:
        st.info(
                """
                #### This menu provides insights like :
                - Rating overall for a specific year and Quarter.
                - Top 10 States, Districts, and Pincodes according to total transactions and phonepe spending.
                - Top 10 States, Districts, and Pincodes depending on the number of phonepe users and how often they open their apps.
                - Top 10 mobile brands and their percentages according to the number of users of phonepe.
                """,icon="🔍"
                )
...
# Top Charts - TRANSACTIONS    
    if Type == "Transactions":
        ...
        with col1:
            ...
            fig.update_layout(title_font_color="#B19CD9")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col2:
            ...
            st.markdown("## :violet[Overall State  - Transactions Count]")
            mycursor.execute(f"select state, sum(count) as Total_Transactions, sum(amount) as Total_amount from map_trans where year = {Year} and quarter = {Quarter} group by state order by state")
            df1 = pd.DataFrame(mycursor.fetchall(),columns= ['State', 'Total_Transactions', 'Total_amount'])
            df2 = pd.read_csv('Statenames.csv')
